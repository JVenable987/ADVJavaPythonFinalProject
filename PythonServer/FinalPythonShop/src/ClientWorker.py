    ...
    def run(self):
        ...
        while self.__keep_running:
            ...
            if "QUIT" in client_message:
                ...
                print("NONNUMERIC")
                self.__client_socket.send("Errored Input".encode("UTF-8"))
            elif int(args[0]) == 0:
                new_customer = Customer(args[1], args[2], args[3])
                self.__server.add_customer_to_customer_list(new_customer)
                self.__client_socket.send("customer added".encode("UTF-8"))
            elif int(args[0]) == 1:
                ...
                self.client_socket.send("item added to cart".encode("UTF-8"))
            elif int(args[0]) == 3:
                ...
                self.client_socket.send("item added".encode("UTF-8"))
            elif int(args[0]) == 4:
                for item in self.__server.catalog.items:
                    self.client_socket.send(str(item).encode("UTF-8"))
            elif int(args[0]) == 5:
                self.__server.get_customer_from_name(args[1]).empty_shopping_cart()
                self.client_socket.send("cart emptied".encode("UTF-8"))
            elif int(args[0]) == 6:
                ...
                for item_to_remove in cart_to_remove_items:
                    if item_to_remove.product_id == int(args[2]):
                        ...
                        self.__client_socket.send("items removed".encode("UTF-8"))
            else:
                self.__client_socket.send("Unknown Command".encode("UTF-8"))
            self.__lock.release()
            #else:
